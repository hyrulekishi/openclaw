...
def default_out_dir(input_value: str, is_url: bool) -> Path:
    if is_url:
        return (Path.cwd() / 'output' / 'translation-run').resolve()
...
def main():
    ...
    p.add_argument('--refine', dest='refine', action='store_true', default=True, help='Run local qwen second-pass refinement (default: on)')
    p.add_argument('--no-refine', dest='refine', action='store_false', help='Disable local qwen second-pass refinement')
    ...
    p.add_argument('--debug', action='store_true', help='Keep intermediate artifacts like normalized.json / bilingual.txt')
    args = p.parse_args()
    ...
    try:
        ...
        emit_cmd = [
            python_bin, str(EMIT),
            '--input', str(translated_path),
            '--out-dir', str(out_dir),
            '--srt-mode', args.srt_mode,
        ]
        if args.debug:
