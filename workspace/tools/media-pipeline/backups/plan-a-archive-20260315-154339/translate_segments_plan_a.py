...
def is_noisy_segment(text: str, translation: str) -> bool:
    ...
    for i in range(1, len(merged)):
        if merged[i] == merged[i - 1]:
            ...
            current_run = 1
    repeated_ratio = len(translation) / max(1, len(text)) if text else 1.0
    return punct_ratio > 0.18 or max_run >= 8 or (len(text) <= 60 and len(translation) >= 220) or (repeated_ratio >= 5.0 and len(translation) >= 180)
        ...
        'messages': [
            {
                ...
                'content': (
                    ...
                    'Return exactly one refined Chinese line per input segment, in the same order as input. '
                    'Do not return JSON. Do not add numbering. Do not add explanations.'
                )
            },
            {
                'role': 'user',
                'content': '\n\n'.join(
                    f"SOURCE:\n{(s.get('text') or '').strip()}\n\nMACHINE_TRANSLATION:\n{(s.get('translation') or '').strip()}"
                    for s in batch
                )
            }
        ]
...
def main():
    p = argparse.ArgumentParser(description='Translate normalized segments (plan A)')
    ...
    try:
        for chunk in batched_by_chars(segs, args.translate_max_chars):
            ...
            if pending:
                ...
                print(f"[translate][plan_a] batch segments={len(pending_texts)} chars={batch_char_count} max_chars={args.translate_max_chars}", file=sys.stderr)
                try:
        ...
        print(f"[translate][plan_a] failed fast after {translate_elapsed_ms} ms: {translate_error or e}", file=sys.stderr)
        raise
    else:
        translate_elapsed_ms = round((time.time() - translate_started_at) * 1000)
    ...
    if args.refine and translated:
        ...
        try:
            print(f"[refine][plan_a] waiting for local model at {args.refine_base_url} (timeout={args.refine_ready_timeout:.0f}s)", file=sys.stderr)
            ...
            refine_chunk_size_used = max((len(c) for c in refine_batches), default=0)
            total_chunks = len(refine_batches)
            for idx, chunk in enumerate(refine_batches, start=1):
                chunk_chars = sum(len((s.get('text') or '').strip()) + len((s.get('translation') or '').strip()) for s in chunk)
                line_results = None
                for attempt in range(1, max(1, args.refine_retries) + 1):
                    started = time.time()
                    print(f"[refine][plan_a] chunk {idx}/{total_chunks} attempt {attempt}/{max(1,args.refine_retries)} size={len(chunk)} chars={chunk_chars} max_chars={args.refine_max_chars} timeout={args.refine_timeout:.0f}s", file=sys.stderr)
                    try:
                        line_results = lmstudio_refine_batch(chunk, source_lang, args.target_lang, args.refine_base_url, args.refine_timeout)
                        elapsed = round((time.time() - started) * 1000)
                        print(f"[refine][plan_a] chunk {idx}/{total_chunks} done in {elapsed} ms lines={len(line_results)}", file=sys.stderr)
                        break
                    except Exception as e:
                        elapsed = round((time.time() - started) * 1000)
                        print(f"[refine][plan_a] chunk {idx}/{total_chunks} attempt {attempt} failed in {elapsed} ms: {e}", file=sys.stderr)
                        if attempt >= max(1, args.refine_retries):
                            raise
                local_changes = 0
                line_results = line_results or []
                for pos, seg in enumerate(chunk):
                    item = dict(seg)
                    if pos < len(line_results) and line_results[pos].strip():
                        new_text = line_results[pos].strip()
                    else:
            ...
            if changed_count > 0:
                refined.sort(key=lambda x: x.get('id', 0))
                translated = refined
                refine_applied = True
            ...
            print(f"[refine][plan_a] skipped: {e}", file=sys.stderr)
            if args.refine_strict:
                raise
        finally:
            refine_elapsed_ms = round((time.time() - refine_started_at) * 1000)
            print(f"[refine][plan_a] total elapsed={refine_elapsed_ms} ms status={refine_status}", file=sys.stderr)
