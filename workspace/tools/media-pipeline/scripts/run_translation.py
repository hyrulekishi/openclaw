...
TRANSLATE = ROOT / 'translate_segments.py'
EMIT = ROOT / 'emit_outputs.py'
COMPOSE = ROOT / 'compose_subtitle_video.py'
...
def default_out_dir(input_value: str, is_url: bool) -> Path:
    if is_url:
        return (Path.cwd() / 'output' / 'translation-run-plan-b').resolve()
    return Path(input_value).expanduser().resolve().parent


def main():
    p = argparse.ArgumentParser(description='Unified normalize -> translate -> emit pipeline')
    ...
    p.add_argument('--compose-video', action='store_true', help='Compose subtitle video after SRT emission')
    p.add_argument('--video-input', default=None, help='Source video path for subtitle video composition')
    p.add_argument('--compose-mode', choices=['burn', 'soft'], default='burn', help='Subtitle composition mode')
    p.add_argument('--composed-video-name', default=None, help='Override composed video filename')
    ...
    if args.debug:
        ...
        fd, tmp_name = tempfile.mkstemp(prefix='normalized-plan-b-', suffix='.json', dir=str(out_dir))
        os.close(fd)
    ...
    try:
        ...
        if args.compose_video:
            video_input = Path(args.video_input).expanduser().resolve() if args.video_input else None
            srt_path = out_dir / 'subtitles.zh.srt'
            if video_input is None:
                raise SystemExit('--compose-video requires --video-input')
            if not srt_path.exists():
                raise SystemExit(f'Cannot compose subtitle video: missing SRT at {srt_path}')
            composed_name = args.composed_video_name or (
                'subtitled.burned.mp4' if args.compose_mode == 'burn' else 'subtitled.softsub.mp4'
            )
            composed_path = out_dir / composed_name
            compose_cmd = [
                python_bin, str(COMPOSE),
                '--video', str(video_input),
                '--srt', str(srt_path),
                '--output', str(composed_path),
